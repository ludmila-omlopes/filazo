...
import type { MarketplaceInput, MarketplaceOffer, MarketplaceResult, MarketplaceSnapshot, MarketplaceSubscription } from "@/lib/assistant/marketplace-search";

const copy = {
  "pt-BR": {
    heading: "Onde comprar", description: "Consulte lojas digitais e catálogos de assinatura nas principais plataformas.",
    region: "Região da loja", search: "Buscar lojas e preços", refresh: "Atualizar preços", searching: "Pesquisando lojas e pré-vendas…",
    ...
    unauthorized: "Entre na sua conta para pesquisar lojas.", refreshLimit: "Este jogo já atingiu o limite de atualizações nas últimas 24 horas. Consulte os dados atuais e tente novamente mais tarde.", refreshing: "Outra atualização deste jogo já está em andamento…",
    ...
    subscriptions: "Incluído em assinaturas", included: "Inclusão confirmada", noSubscriptions: "Nenhuma inclusão em assinatura foi confirmada nesta busca.", tier: "Plano",
  },
  en: {
    heading: "Where to buy", description: "Check digital stores and subscription catalogs across the main platforms.",
    region: "Store region", search: "Find stores and prices", refresh: "Update prices", searching: "Searching stores and preorders…",
    ...
    unauthorized: "Sign in to search stores.", refreshLimit: "This game has reached its refresh limit for the last 24 hours. Check the current data and try again later.", refreshing: "Another refresh for this game is already in progress…",
    ...
    subscriptions: "Included with subscriptions", included: "Inclusion confirmed", noSubscriptions: "No subscription inclusion was confirmed in this search.", tier: "Tier",
  },
};

export function MarketplaceSearchPanel({ title, gameId, initialSnapshots, onUsePrice, onRegionChange }: {
  title: string;
  gameId?: string;
  initialSnapshots?: MarketplaceSnapshot[];
  ...
  const initialSnapshot = initialSnapshots?.find((snapshot) => snapshot.region === region) ?? null;
  return (
    <section className="grid gap-3 rounded-inner border border-edge bg-canvas/60 p-4" aria-label={text.heading}>
      ...
      <MarketplaceResults key={JSON.stringify([title.trim(), region, locale])} gameId={gameId} initialResult={initialSnapshot} input={{ title: title.trim(), region, locale }} onUsePrice={onUsePrice} />
    </section>
  );
}

function MarketplaceResults({ gameId, initialResult, input, onUsePrice }: { gameId?: string; initialResult: MarketplaceSnapshot | null; input: MarketplaceInput; onUsePrice?: (price: string) => void }) {
  const text = copy[input.locale];
  const [result, setResult] = useState<MarketplaceResult | null>(initialResult);
  ...
  async function search() {
    ...
    try {
      const response = await fetch("/api/assistant/marketplaces", {
        ...
        body: JSON.stringify(gameId ? { ...input, gameId } : input), signal: requestController.signal,
      });
      if (!response.ok) {
        const payload = await response.json().catch(() => ({}));
        const message = payload.code === "NOT_CONFIGURED" ? text.unavailable
          : payload.code === "AI_LIMIT" ? text.limit
          : payload.code === "GAME_REFRESH_LIMIT" ? text.refreshLimit
  ...
  return (
    <div className="grid min-w-0 gap-3">
      <Button type="button" variant="outline" className="justify-self-start" disabled={loading || input.title.length < 2} loading={loading} onClick={search}>{result ? text.refresh : text.search}</Button>
      <div aria-live="polite" role="status" className="text-sm">
        {loading ? text.searching : error || (result && !result.offers.length && !result.subscriptions.length && !result.partial ? text.empty : null)}
      </div>
      {result?.refreshing && !loading ? <p className="text-sm text-ink-soft" role="status">{text.refreshing}</p> : null}
              ...
              return (
                <li key={offer.url} className="grid min-w-0 gap-2 rounded-inner border border-edge bg-surface p-3">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {price && onUsePrice ? <Button type="button" variant="secondary" size="sm" onClick={() => onUsePrice(`${offer.currency} ${offer.price!.toFixed(2)}`)}>{text.use}</Button> : null}
                  </div>
                </li>
              );
            })}
      ...
      {result && (!result.refreshing || result.offers.length || result.subscriptions.length) ? <p className="text-xs leading-relaxed text-ink-soft">{text.checked} <time dateTime={result.checkedAt}>{new Date(result.checkedAt).toLocaleString(input.locale)}</time>. {text.note}</p> : null}
      {result ? <SubscriptionResults refreshing={Boolean(result.refreshing && !result.offers.length && !result.subscriptions.length)} subscriptions={result.subscriptions} text={text} /> : null}
    </div>
  );
}
...
function SubscriptionResults({
  ...
  refreshing: boolean;
  text: (typeof copy)[MarketplaceInput["locale"]];
}) {
  return (
    <section className="grid gap-3 rounded-inner border border-edge bg-canvas/60 p-4" aria-label={text.subscriptions}>
      <div>
        <h4 className="font-semibold">{text.subscriptions}</h4>
        <p className="mt-1 text-sm text-ink-soft">{refreshing ? text.refreshing : subscriptions.length ? text.included : text.noSubscriptions}</p>
      </div>
      {subscriptions.length ? (
        <ul className="grid gap-3">
          {subscriptions.map((subscription) => (
            <li key={subscription.url} className="grid min-w-0 gap-2 rounded-inner border border-edge bg-surface p-3">
              <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                <strong className="break-words">{subscription.service}</strong>
                <span className="text-xs font-semibold text-ink-soft">{text.included}</span>
              </div>
              <p className="break-words text-sm">
                {[subscription.title, subscription.tier ? `${text.tier}: ${subscription.tier}` : null].filter(Boolean).join(" · ")}
              </p>
              <p className="break-words text-xs leading-relaxed text-ink-soft">{subscription.evidence}</p>
              <a className="inline-flex min-h-11 items-center text-sm font-semibold underline underline-offset-4" href={subscription.url} target="_blank" rel="noopener noreferrer">
                {text.visit} ↗
              </a>
            </li>
          ))}

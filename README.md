...
Steam and GOG imports use the same durable background queue. The Sources action saves a `PlatformSyncRun` and returns immediately; users can navigate away or close the tab. A server worker commits canonical resolution, ownership and the cursor together for each game. Runs use bounded batches with expiring leases, automatic backoff and recovery after a process restart. Duplicate clicks and scheduled requests reuse the active run. After repeated failures, a manual retry resumes the saved snapshot in a new audit run. Disconnecting the account cancels its queued work. Optional metadata enrichment uses `GameMetadataJob`; new-title matching can still consult the metadata provider during canonical resolution.

Steam saves its library response once. GOG saves only the current page, its offset in the import and the next page number in the existing snapshot JSON. A failed page request retries that page; an interrupted game import resumes within the saved page. There is no 100-page ceiling or whole-library timeout. Each batch processes at most 100 games within a 35-second work budget, with a 90-second lease. No new database migration is needed beyond the existing background-sync and GOG schema. GOG uses live alphabetical pagination, so games added or removed while an import is running can shift page boundaries; later refreshes reconcile ownership without deleting existing entries.

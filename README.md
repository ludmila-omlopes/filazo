...
Journal image tokens are capped at **10 MiB**, audio tokens use the configured audio limit, and size is checked again when attaching media. Tokens expire in five minutes and cannot overwrite files or select random suffixes. Quotas count token authorizations, including abandoned uploads, not successful saves. Upload/delete JSON is capped at 8 KiB while streaming; assistant chat at 128 KiB and the other assistant APIs at 8 KiB. Next.js's default 1 MiB Server Action body limit remains in place.

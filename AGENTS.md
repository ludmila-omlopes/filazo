...
- Keep the primary catalog interface calm and low-busy. Secondary information and less-frequent actions should use progressive disclosure: separate tabs, filters, details panels, mode buttons such as "Update status", expandable sections, or other controls that reveal functionality only when needed.
- Avoid prominent counts, totals, and numerical summaries that can make the library feel like a backlog obligation. Do not surface total game counts, backlog size, owned/wishlist totals, or similar anxiety-inducing numbers in calm overview surfaces unless the user explicitly asks for them or the number is required to complete a focused task.
